...
import { useBookingForm } from '@/features/shared/hooks/useBookingForm';
...
export default function BookingForm({ room }: { room: RoomType }) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await submitBooking(room.id, data);
      setOpenModal(true);
    } catch (error) {
      console.error('Error submitting booking:', error);
      alert(error instanceof Error ? error : 'Có lỗi xảy ra khi đặt phòng!');
    }
  };
